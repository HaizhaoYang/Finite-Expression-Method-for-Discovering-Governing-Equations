...
def train(
        ...
        partial_constants,
        partial_rnn,
        operator_list = ['*', '+', '-', '/', '^', 'cos', 'sin', 'c', 'var_x','exp','ln'],
        min_length = 2,
        max_length = 12,
        type = 'lstm',
        num_layers = 1,
    ...
    dsr_rnn = DSRRNN(operators, hidden_size, device, min_length=min_length,
                     ...
                     apply_constraint = apply_constraint).to(device)
    if (optimizer == 'adam'):
    ...
    start = time.time()
    sequences, sequence_lengths, entropies, log_probabilities = dsr_rnn.sample_sequence(initial_batch_size, min_length, max_length, apply_constraint)
    #print(sequences[:10])
    ...
    for i in range(num_batches):
        ...
        optimize_constants(expressions, X_constants, y_constants, u_obs_constans,partial_constants,inner_lr, inner_num_epochs, inner_optimizer)

        # Benchmark expressions (test dataset)
        rewards = []
        for expression in expressions:
            rewards.append(benchmark(expression, X_rnn, y_rnn, u_obs_rnn,partial_rnn))
        ...
        sequences, sequence_lengths, entropies, log_probabilities = dsr_rnn.sample_sequence(batch_size, min_length, max_length, apply_constraint)
    ...
    if (summary_print):
        ...
        Best Performance: {1/best_performance-1}
...
def benchmark(expression, X_rnn, y_rnn, u_obs_rnn, partial_rnn):
    ...
    with torch.no_grad():
        ...
        # print('X_rnn shape:')
        # print(X_rnn.shape)
        y_pred = (y_pred+partial_rnn)*1e-2 + u_obs_rnn
